...
#               2017-2020 Unknown Open Source Project
#
...
function setup_vendor() {
    ...
    export UOSP_ROOT="$3"
    if [ ! -d "$UOSP_ROOT" ]; then
        echo "\$UOSP_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$UOSP_ROOT/$OUTDIR" ]; then
        mkdir -p "$UOSP_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$UOSP_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDBP="$UOSP_ROOT"/"$OUTDIR"/Android.bp
    export ANDROIDMK="$UOSP_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$UOSP_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function write_blueprint_header() {
    ...
    if [ $BLUEPRINT_INITIAL_COPYRIGHT_YEAR -eq $YEAR ]; then
        printf " * Copyright (C) $YEAR Unknown Open Source Project\n" >> $1
    elif [ $BLUEPRINT_INITIAL_COPYRIGHT_YEAR -le 2019 ]; then
        printf " * Copyright (C) 2019-$YEAR Unknown Open Source Project\n" >> $1
    else
        printf " * Copyright (C) $BLUEPRINT_INITIAL_COPYRIGHT_YEAR-$YEAR Unknown Open Source Project\n" >> $1
    fi
...
function write_makefile_header() {
    ...
    if [[ $INITIAL_COPYRIGHT_YEAR =~ $NUM_REGEX ]] && [ $INITIAL_COPYRIGHT_YEAR -le $YEAR ]; then
        ...
        if [ $YEAR -eq 2017 ]; then
            printf "# Copyright (C) 2017 Unknown Open Source Project\n" >> $1
        elif [ $INITIAL_COPYRIGHT_YEAR -eq $YEAR ]; then
            printf "# Copyright (C) $YEAR Unknown Open Source Project\n" >> $1
        elif [ $INITIAL_COPYRIGHT_YEAR -le 2017 ]; then
            printf "# Copyright (C) 2017-$YEAR Unknown Open Source Project\n" >> $1
        else
            printf "# Copyright (C) $INITIAL_COPYRIGHT_YEAR-$YEAR Unknown Open Source Project\n" >> $1
        fi
    else
        printf "# Copyright (C) $YEAR Unknown Open Source Project\n" > $1
    fi
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$UOSP_ROOT"/prebuilts/tools-uosp/common/smali/baksmali.jar
        export SMALIJAR="$UOSP_ROOT"/prebuilts/tools-uosp/common/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$UOSP_ROOT"/prebuilts/tools-uosp/${HOST}-x86/bin/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$UOSP_ROOT"/prebuilts/tools-uosp/${HOST}-x86/bin/compact_dex_converter
    fi
    ...
    if [ ! -f "$UOSP_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$UOSP_TARGET" >/dev/null; then
...
function extract() {
    ...
    local OUTPUT_ROOT="$UOSP_ROOT"/"$OUTDIR"/proprietary
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            do
                ...
                if [ -a "$DUMPDIR"/"$PARTITION".new.dat ]; then
                    ...
                    python "$UOSP_ROOT"/vendor/uosp/build/tools/sdat2img.py "$DUMPDIR"/"$PARTITION".transfer.list "$DUMPDIR"/"$PARTITION".new.dat "$DUMPDIR"/"$PARTITION".img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$UOSP_ROOT"/"$OUTDIR"/radio
