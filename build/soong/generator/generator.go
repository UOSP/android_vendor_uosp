...
// Copyright (C) 2018 The UospOS Project
//
...
func init() {
	android.RegisterModuleType("uosp_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = uospExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = uospExpandVariables(ctx, dep_file)
	...
	cmd := uospExpandVariables(ctx, String(g.properties.Cmd))
